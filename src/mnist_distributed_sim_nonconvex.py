...
class MNISTConvNet(object):
    ...
    def construct_model(self):
        ...
        with tf.variable_scope(self.model_name):
            ...
            # model parameters
            #  model: input -> conv(32) -> conv(32) -> dense(256) ->  softmax( W^T x + b ) -> y_hat
            ...
            network = tf.reshape(self.x, [-1, 28, 28, 1], name='shape_as_image')

            # layer 0 : conv layer 0
            with tf.variable_scope('conv_layer_0'):
                network = tf.add(conv2d(network, self.W_list[0]), self.b_list[0])
                network = tf.nn.relu(network)
                network = max_pool_2x2(network)

            # layer 1 : conv layer 1
            with tf.variable_scope('conv_layer_1'):
                network = tf.add(conv2d(network, self.W_list[1]), self.b_list[1])
                network = tf.nn.relu(network)
                network = max_pool_2x2(network)

            network = tf.reshape(network, [-1, self.W_shape_list[2][0]], name='flatten_to_vector')

            # layer 2 : dense layer 0
            with tf.variable_scope('dense_layer_0'):
                network = tf.add(tf.matmul(network, self.W_list[2]), self.b_list[2])
                network = tf.nn.relu(network)

            # layer 3 : softmax output
            with tf.variable_scope('softmax_layer'):
                network = tf.add(tf.matmul(network, self.W_list[3]), self.b_list[3])
...
class DistSimulation(MNISTConvNet):
    ...
    def train_distributed_models(self):
        ...
        for i_machine, mnist_train in enumerate(self.training_data_sets):
            ...
            model = MNISTConvNet(self.minibatch_size, self.learning_rate, self.averaging_interval, 
...
if __name__=='__main__':
    ...
    mnist_classifier = MNISTConvNet(minibatch_size, learning_rate, 
